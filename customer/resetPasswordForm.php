...
<body>
  <div class="background-img d-flex align-items-center justify-content-center flex-column">
    ...
    <div class="container-fluid mt-3 d-flex flex-column align-items-center">
      <form action="../customer-database/resetPassword.php" method="POST">
        <input type="hidden" name="token" value="<?= htmlspecialchars($_GET['token']) ?>">
        <div class="form-outline">
          <label for="password" class="form-label">Enter New Password</label>
          <input type="password" class="form-control rounded-4 py-2 mb-4 bg-transparent border-2" name="password"
            id="password" required />
        </div>
        <div class="form-outline">
          <label for="confirmPassword" class="form-label">Confirm New Password</label>
          <input type="password" class="form-control rounded-4 py-2 mb-4 bg-transparent border-2" name="confirmPassword"
